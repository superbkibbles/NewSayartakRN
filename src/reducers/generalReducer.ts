...
export const generalReducer = createReducer(initialState, {
  ...
  [types.FORGOT_PASSWORD_SEND_OTP](state, action) {
    return {
      ...state,
      verify_phone_token: action.response.token,
    };
  },
  ...
  [types.GET_SHOW_ROOM_CARS](state, action) {
    ...
    return {
      ...
      showroomsCars_nextPage: page,
      isThare_showroomsCarsPages: action.response.next ? true : false,
    };
  },
  //Save data
  [types.SIMILAR_CARS](state, action) {
    return {
      ...state,
      similr_cars: action.response.results,
    };
  },
  ...
  [types.GET_BRAND_MODALS](state, action) {
    ...
    return {
      ...state,
      brandModals: action.response.results,
    };
  },
  ...
  [types.GET_ALL_FILTER_CARS](state, action) {
    ...
    return {
      ...state,
      resultFilterCars: [...state.resultFilterCars, ...action.response.results],
      resultFilterCars_nextPage: getNexPage(
        action.response.next,
        state.resultFilterCars_nextPage,
      ),
      isThareResultFilterCarsPage: action.response.next ? true : false,
    };
  },
  //advanced search
  [types.SET_ADVANCED_FILTER](state, action) {
    return {
      ...state,
      [action.filterKey]: { ...state[action.filterKey], ...action.data },
    };
  },
  ...
  [types.REQUEST_ACTION](state, action) {
    if (action.payload.reset)
      return {
        ...state,
        [action.payload.reset]: defaultData[action.payload.reset],
      };
    else return { ...state };
  },
});
