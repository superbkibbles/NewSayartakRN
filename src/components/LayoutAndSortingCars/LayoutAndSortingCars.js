 ...
 function LayoutAndSortingCars(params: LayoutAndSortingCarsProps) {
    ...
    return (
        <ViewRow style={{ paddingVertical: calcHeight(6) }}>
            <ViewRow
                style={{ width: calcWidth(70), backgroundColor: config.colors.WHITE, paddingHorizontal: calcWidth(10), height: calcHeight(35), alignItems: "center", borderWidth: 1, borderColor: config.colors.BORDER_COLOR, borderRadius: calcHeight(35 / 2) }}
            >
                <Button
               onPress={()=>onPress("big")}
                    iconStyle={{ color: layout === "big" ? config.colors.BASE_COLOR : config.colors.GRAY, width: RFValue(13, 812), height: RFValue(13, 812) }}
                ...
                <Button
                    ...
                    style={{ height: "100%", alignItems: "center" }}
                    iconStyle={{ color: layout === "small" ? config.colors.BASE_COLOR : config.colors.GRAY, width: RFValue(13, 812), height: RFValue(13, 812) }}
                    iconName={"listLayout"}
                    textStyle={{ fontSize: "10", color: config.colors.BLACK }}
            ...
            <Button
                ...
                iconStyle={{ color: config.colors.BASE_COLOR, width: RFValue(12, 812), height: RFValue(12, 812) }}
                style={{ backgroundColor: config.colors.WHITE, paddingHorizontal: calcWidth(10), height: calcHeight(35), alignItems: "center", borderWidth: 1, borderColor: config.colors.BORDER_COLOR, borderRadius: calcHeight(35 / 2) }}
                text={params.selectedIndex != -1 ? params.data[params.selectedIndex].name : "رتب حسب"}
                iconName={params.selectedIndex != -1 ? params.data[params.selectedIndex].iconName : undefined}
                textStyle={{ fontSize: "10", color: config.colors.BLACK }}
            />
        </ViewRow>
    )
...
function mapStateToProps(state) {
  ...
  function mapDispatchToProps(dispatch) {
    return {
      changeLayout:(layout,component_type) => dispatch(Actions.changeLayout(layout,component_type)),
