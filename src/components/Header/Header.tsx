...
export function Header(params: HeaderProps) {
  ...
  const onNotification = () => {
    if (token) NavigationService.navigate('Notification');
    else NavigationService.navigate('SignIn');
  };
  ...
  if (params.y) {
    bottom = params.y.interpolate({
      inputRange: [0, calcHeight(200)],
      outputRange: [0, calcHeight(20)],
      extrapolate: 'clamp',
      easing: Easing.elastic(),
    });
    _height = params.y.interpolate({
      inputRange: [0, calcHeight(200)],
      outputRange: [height, height - 30],
      extrapolate: 'clamp',
      easing: Easing.elastic(),
    });
    gradientHeight = params.y.interpolate({
      inputRange: [0, calcHeight(200)],
      outputRange: [calcHeight(107), calcHeight(96)],
      extrapolate: 'clamp',
      easing: Easing.elastic(),
    });
    opacity = params.y.interpolate({
      inputRange: [0, calcHeight(200)],
      outputRange: [1, 0],
      extrapolate: 'clamp',
      easing: Easing.elastic(),
  ...
  return (
    <Animated.View
      style={[
        { width: '100%', alignItems: 'center', zIndex: 100, height: _height },
        params.style,
      ]}
    >
      <GradientView
        style={{
          ...
          height:
            params.style && params.style.height
              ? params.style.height
              : gradientHeight,
          borderBottomEndRadius: RFValue(10, 812),
          borderBottomStartRadius: RFValue(10, 812),
          alignItems: 'center',
      ...
      >
        <Animated.View
          style={{
            marginTop: calcHeight(params.buttons || params.onSearch ? 10 : 30),
            opacity: opacity,
            flexDirection: 'row',
          ...
          >
            {!params.onBack && (
              <Badge
                ...
                style={{ right: calcWidth(10), top: -calcHeight(5) }}
              />
            )}
          </Button>
        </Animated.View>
      ...
      >
        {params.buttons && <TabButtons items={params.buttons} />}
        {params.onSearch && (
          <InputSearch onChangeText={params.onSearch} style={{}} />
        )}
      </Animated.View>
...
export function HeaderWithShare(params: HeaderProps) {
  ...
  return (
    <View
      style={{ position: 'absolute', width: '100%', height: calcHeight(130) }}
    >
      <LinearGradient
        end={{ x: 1, y: 0 }}
        start={{ x: 1, y: 1 }}
      ...
      >
        <ViewRow style={{ width: calcWidth(50), paddingHorizontal: 0 }}>
          ...
          <Button
            onPress={onFavourite}
            iconName={isFavourite ? 'activeStar' : 'star'}
            iconStyle={{
              color: config.colors.WHITE,
              width: RFValue(21, 812),
              height: RFValue(21, 812),
            }}
          />
        </ViewRow>
        <Button
