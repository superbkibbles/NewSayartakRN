...
class SwipeablePanel extends Component {
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (gestureState.dy == 0) this._animateTo(this.state.status);
          ...
          if (this.state.status == STATUS.SMALL)
            this._animateTo(onlySmall ? STATUS.SMALL : STATUS.SMALL);
  ...
  componentDidMount = () => {
    ...
    this.state.pan.y.addListener(value => {
      return (this.animatedValueY = value.value);
    });
    ...
    if (newStatus == STATUS.CLOSED) newY = PANEL_HEIGHT;
      ...
      newY =
        this.state.orientation === 'portrait'
          ? FULL_HEIGHT - this.props.height || 400
          : FULL_HEIGHT / 3;
  ...
  render() {
    const { showComponent, deviceWidth, deviceHeight, panelHeight } =
      this.state;
    const {
      noBackgroundOpacity,
      style,
    ...
    return showComponent ? (
      <Animated.View
        style={[
          SwipeableModalStyles.background,
          {
        ...
        >
          ...
          <ScrollView
            ...
            contentContainerStyle={
              SwipeableModalStyles.scrollViewContentContainerStyle
            }
          >
...
const SwipeableModalStyles = StyleSheet.create({
  background: {
    position: 'absolute',
    zIndex: 1,
    bottom: 0,
